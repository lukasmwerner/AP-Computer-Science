...
public class MagicBox {
  ...
  public static void main(String[] args) {
    System.out.println(
      isMagicBox(new int[][] { { 7, 0, 5 }, { 2, 4, 6 }, { 3, 8, 1 } })
    );
    int[][] user = parseMagic();
  ...
  public static boolean isMagicBox(int[][] box) {
    int magicConstant = (int) (
      box.length * (((box.length * box.length) + 1.0) / (2.0))
    );
    for (int[] row : box) {
      int sum = 0;
      for (int item : row) {
  ...
  public static int[][] parseMagic() {
    ...
    int[][] m = new int[Integer.parseInt(values[0])][Integer.parseInt(
      values[1]
    )];
